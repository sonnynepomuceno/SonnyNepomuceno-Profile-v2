...
import { Monitor, ClipboardCheck, Send, Zap, LayoutDashboard, Lock } from "lucide-react";
          ...
          {projects.map((project, i) => (
            ...
            >
              <BentoGridItem
                ...
                header={
                  <div className={`relative w-full h-24 rounded-xl bg-gradient-to-br ${project.gradient} flex items-center justify-center border border-white/5`}>
                    <div className={`w-12 h-12 rounded-xl ${project.iconBg} flex items-center justify-center border border-white/10`}>
                      {project.icon}
                    </div>
                    <div className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-black/40 border border-white/10 text-neutral-400">
                      <Lock size={10} />
                      <span className="text-[10px] font-medium">NDA</span>
                    </div>
                  </div>
                }
                icon={project.icon}
